...
/*
    These Jetpack Compose functions facilitate UI development in Android apps:

    UserProfilePic: Displays a user profile picture asynchronously, with a fallback icon based on gender.

    RunningStatsItem: Represents a single item in the running statistics list, showing an icon and corresponding value.

    RunItem: Displays a single run item with an image, date, distance, calories burned, and average speed.

    DropDownList: Creates a dropdown menu to select items from a list with customizable string transformation.

    LocationPermissionRequestDialog: Shows a dialog requesting location permission, allowing users to grant or deny access.
 */
@Composable
fun UserProfilePic(
